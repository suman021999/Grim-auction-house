...
const history = {
  active: all
    .filter(i => i.auctionStatus === "Active" && i.adminStatus === "Approved").map(formatItem),
  ...
  ended: all
    .filter(i => i.auctionStatus === "Ended" && i.adminStatus === "Approved").map(formatItem),
};
...
const summary = {
  ...
  revenue: history.ended.reduce((acc, cur) => acc + (cur.amountBid ?? 0), 0),
};

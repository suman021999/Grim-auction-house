...
const StatusBadge = ({ status }) => {
  ...
  if (s === "winning")
    return (
      <span className={`${base} bg-green-500 text-amber-50`}> {status} </span>
    );
  if (s === "outbid")
    return (
      <span className={`${base} bg-red-500 text-amber-50`}> {status} </span>
    );
  if (s === "pending")
    return (
      <span className={`${base} bg-yellow-500 text-amber-50`}> {status} </span>
    );
  if (s === "ended")
    return (
      <span className={`${base} bg-gray-500 text-amber-50`}> {status} </span>
    );
        ...
        <main className="flex-1 p-4 md:p-8">
          ...
          <div className="mb-6 flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
            ...
            <div className="flex gap-3 md:hidden">
              ...
              <SummaryCard
                title="Revenue"
                value={data.summary?.revenue ? `$${data.summary.revenue}` : "—"}
              />
            </div>
          </div>
                ...
                <table className="w-full min-w-[700px]">
                  ...
                  <tbody>
                    ...
                    {items.map((row) => (
                      <tr
                        key={row.id}
                        className="align-top border-t last:border-b"
                      >
                        <td className="py-4">
                        ...
                        <td className="py-4">
                          <div className="text-sm text-black truncate max-w-[140px]">
                            {row.name}
                          </div>
                        </td>
                ...
                <div className="bg-white  p-4 rounded-2xl shadow-sm hidden md:block">
                  ...
                  <div className="mt-4 flex flex-col gap-3">
                    ...
                    <SummaryCard
                      title="Revenue"
                      value={
                        data.summary?.revenue ? `$${data.summary.revenue}` : "—"
                      }
                    />
                  </div>
                </div>
                ...
                {activeTab === "review" && (
                  <div className="bg-white p-4 rounded-2xl shadow-sm">
                    ...
                    <div className="flex flex-col gap-2">
                      <button className="w-full py-2 text-black rounded-lg border">
                        Approve selected
                      </button>
                      <button className="w-full py-2 text-black rounded-lg border">
                        Remove selected
